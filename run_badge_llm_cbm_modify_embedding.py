...
修改说明：
- 修改了 compute_gradient_embeddings 函数，使其包含 CBM 损失函数的完整梯度嵌入
- 根据公式：g(x) = W_y^T (p - e_y) + W_c^T [α_concept (σ(z) - c) + (λ/C) sign(z)]
- 使用选项B：连接两个头的梯度嵌入
...
class CBMHead(nn.Module):
    ...
    """
    def __init__(self, hidden_size=768, num_labels=4, n_concepts=24, hidden_task=256, dropout=0.1,
...
# ----------------------------
@torch.no_grad()
def compute_gradient_embeddings(encoder, head, loader, device, alpha_concept=1.0) -> Tuple[np.ndarray, List[int]]:
    """
    计算 BADGE 梯度嵌入（包含 CBM 损失的完整项）
    
    根据公式：
    g(x) = W_y^T (p - e_y) + W_c^T [α_concept (σ(z) - c) + (λ/C) sign(z)]
    
    其中：
    - W_y 是分类头的权重矩阵
    - W_c 是概念预测层的权重矩阵
    - p 是类别上的 softmax 概率分布
    - e_y 是预测类别的 one-hot 向量
    - σ(z) 是概念 logits 的 sigmoid（即 c）
    - c 是真实概念标签（如果存在，否则使用预测值）
    - α_concept 是概念损失的权重
    - λ/C 是 L1 正则化项（对应 concept_l1）
    - sign(z) 是 z 的符号函数
    ...
    for batch in loader:
        ...
        # 前向传播，获取 logits、概念和概念 logits
        logits, c, z_c = head(h, return_concepts=True)  # logits: [B, K], c: [B, C], z_c: [B, C]
        ...
        delta_y = probs - onehot          # [B, K] 概率与 one-hot 的差
        # 分类头梯度嵌入：外积后展平
        g_y = (delta_y.unsqueeze(-1) * h.unsqueeze(1)).reshape(probs.size(0), -1)  # [B, K*H]
        
        # ========== 计算概念预测层的梯度嵌入 ==========
        # 获取真实概念标签（如果存在），否则使用预测值
        c_targets = batch.get("concepts", None)
        if c_targets is not None:
            c_targets = c_targets.float()  # [B, C]
        else:
            # 如果没有真实概念标签，使用预测值（在 BADGE 中通常没有真实标签）
            c_targets = c  # [B, C]
        
        # 计算概念损失项：α_concept * (σ(z) - c)
        concept_loss_term = alpha_concept * (c - c_targets)  # [B, C]
        
        # 计算 L1 正则化项：(λ/C) * sign(z)
        # 注意：这里 λ/C 对应 head.concept_l1，C 是概念数量
        n_concepts = z_c.size(1)
        l1_term = (head.concept_l1 / n_concepts) * torch.sign(z_c)  # [B, C]
        
        # 概念预测层的梯度项
        delta_c = concept_loss_term + l1_term  # [B, C]
        
        # 概念预测层梯度嵌入：外积后展平
        g_c = (delta_c.unsqueeze(-1) * h.unsqueeze(1)).reshape(delta_c.size(0), -1)  # [B, C*H]
        
        # ========== 连接两个头的梯度嵌入（选项B）==========
        g = torch.cat([g_y, g_c], dim=1)  # [B, K*H + C*H]
...
def main():
    ...
    for r in range(args.rounds):
        ...
        # 注意：这里传递 alpha_concept 参数
        embs, map_idx = compute_gradient_embeddings(encoder, head, probe_loader, device, alpha_concept=args.alpha_concept)
        if embs.shape[0] == 0: 
            break
    ...
    result_dir = f"results/{config['result_dir']}/llm_cbm_modify_embedding"
